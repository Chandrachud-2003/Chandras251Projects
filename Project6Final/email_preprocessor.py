...
def tokenize_words(text, remove_stop_words=False):
    '''Transforms an email into a list of words, excluding stop words.
    ...
    Returns:
    -----------
    Python list of str. Words in the sentence `text`, excluding stop words.
...
def count_words(email_path='data/enron', remove_stop_words=False):
    ...
    for root, dirs, files in os.walk(email_path):
        ...
        for file in files:
            # Read email file as a bytes sequence
            with open(os.path.join(root, file), 'rb') as f:
                email_bytes = f.read()

            # Decode the bytes sequence as UTF-8
            email_str = email_bytes.decode('utf-8', errors='ignore')
                
            # Tokenize the email into words
            words = tokenize_words(email_str, remove_stop_words=remove_stop_words)
...
def make_feature_vectors(top_words, num_emails, email_path='data/enron', remove_stop_words=False):
    ...
    for folder_name in os.listdir(email_path):
        ...
        for file_name in os.listdir(folder_path):
            ...
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                text = f.read()
                words = tokenize_words(text, remove_stop_words=remove_stop_words)
